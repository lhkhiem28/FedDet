...
class Client(fl.client.NumPyClient):
    ...
    def fit(self, 
        ...
        self.model.train()
        if self.round <= int(0.1*self.num_rounds):
            for param_group in self.optimizer.param_groups:
                param_group["lr"] = self.model.hyperparams["lr"]*self.round/(int(0.1*self.num_rounds))
        else:
        ...
        self.round += 1
        return self.get_parameters(config = {}), len(self.fit_loaders["fit"].dataset), results
...
if __name__ == "__main__":
    ...
    lr_scheduler = optim.lr_scheduler.CosineAnnealingLR(
        optimizer, 
        eta_min = 0.01*model.hyperparams["lr"], T_max = int(0.9*args.num_rounds), 
    )
